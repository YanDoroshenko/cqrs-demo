...
class ReportTaxService (topicName: String, kafkaBootstrapServers: String) {
  ...
  def register() = {
    ...
    props.put(ConsumerConfig.GROUP_ID_CONFIG, "report-tax")
    ...
    .flatMap(records => records.map(r => IO.println(s"Report tax: ${r.value()}")).sequence)
    .recoverWith {
      case ex =>
