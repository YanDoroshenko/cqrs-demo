...
class SendEmailService(topicName: String, kafkaBootstrapServers: String) {
  ...
  def register() = {
    ...
    props.put(ConsumerConfig.GROUP_ID_CONFIG, "send-email")
    ...
    .flatMap(records => records.map(r => IO.println(s"Email sent: ${r.value()}")).sequence)
    .recoverWith {
      case ex =>
